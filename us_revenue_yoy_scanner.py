...
輸出:data/美股季營收年增掃描.xlsx
  ...
  - 分頁2「財報指標」(WITH_PRICE=True 時為「財報與股價」):一檔一列,財報三表科目與比率
    (+股價估值)。預設只出財報(WITH_PRICE=False,因多數受限網路政策會擋 stooq.com)。
...
WITH_PRICE      = False           # 抓股價(Stooq,免金鑰)算市值/PE/PB/PS。預設關:多數受限網路政策
                                  # 會擋 stooq.com;要開請先在環境網路政策放行 stooq.com。
...
def main():
    ...
    fin_sheet = "財報與股價" if WITH_PRICE else "財報指標"
    with pd.ExcelWriter(OUTPUT, engine="openpyxl") as xw:
        df.to_excel(xw, sheet_name="美股季營收年增掃描", index=False)
        # 第二分頁:財報三表科目(+股價估值,若 WITH_PRICE);一檔一列,與第一頁同序
        if not df.empty and (WITH_FINANCIALS or WITH_PRICE):
            ...
            fdf.to_excel(xw, sheet_name=fin_sheet, index=False)
    print(f"\n完成 → {OUTPUT}({len(df)} 檔)"
          + (f";另含『{fin_sheet}』分頁" if (WITH_FINANCIALS or WITH_PRICE) else ""))
