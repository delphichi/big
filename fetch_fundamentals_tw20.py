...
=======================================================================
針對「月營收年增掃描」篩出、最值得深入研究的 20 檔,抓三大表 + 每日 PER/PBR/殖利率,
算出經營績效與估值,輸出『一檔一列』的跨檔比較表(可直接排序比較),另含每檔明細分頁。

資料來源:FinMind(taiwan_stock_financial_statement / balance_sheet /
          cash_flows_statement / per_pbr / month_revenue)。
輸出   :data/台股精選20_財報估值.xlsx

每檔算出(以最新季與近四季為主):
  經營績效 → 毛利率 / 營益率 / 淨利率(近四季平均)、近四季EPS、近四季ROE
  財務結構 → 負債比、流動比(最新季)
...
START_DATE = "2023-01-01"                 # 取 3 年:近四季 + 年增基期都夠
...
def main():
    ...
    for i, (sid, name) in enumerate(PICKS, 1):
        ...
        for attempt in range(3):
            try:
                ...
                row, perf = summary_row(sid, name, raw)
                rows.append(row)
    ...
    cols = ["代號", "名稱", "最新季", "毛利率%", "營益率%", "淨利率%", "近四季EPS", "近四季ROE%",
    ...
    if "近四季ROE%" in df.columns:
        df = df.sort_values("近四季ROE%", ascending=False, na_position="last")
